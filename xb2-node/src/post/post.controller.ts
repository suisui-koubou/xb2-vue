...
import { getPosts, createPost, updatePost, getPostById, getPostsTotalCount, deletePostImp} from './post.service';
...
export const deletePost = async (
  ...
  const { postId } = request.params; 
  try {
    console.log("正在删除", postId);
    const res = await deletePostImp(parseInt(postId, 10)); 
    response.send(res); // 要返回response，不然会404
    next(); 
  }catch(error){
    next(error); 
  }
